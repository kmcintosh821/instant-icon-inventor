...
class Square extends Element {
    ...
    render() {
        const center = (this.sideLength / 2);
        const x = 150 - center;
        const y = 100 - center;
        return `<rect x="${x}" y="${y}" rx="${this.cornerRoundness}" ry="${this.cornerRoundness}" width="${this.sideLength}" height="${this.sideLength}" fill="${this.color}"/>`
    }
